...
function init() {
	...
	var elementList = document.getElementsByClassName("add_to_cart");
	for(i=0;i<elementList.length;i++) {
		elementList[i].addEventListener("click", addToCart);
		elementList[i].addEventListener("mouseout", addToCartMouse);
	}
...
function addToCart() {
	...
	for(i=0;i<menuItem.childNodes.length;i++) {
		if(menuItem.childNodes[i].className == "menu_item_name") {
			name = menuItem.childNodes[i].innerHTML;
		}
		if(menuItem.childNodes[i].className == "pop_up") {
			var span_pop_up = menuItem.childNodes[i];
			for(j=0;j<span_pop_up.childNodes.length;j++) {
				if(span_pop_up.childNodes[j].className == "menu_item_price") {
					price = span_pop_up.childNodes[j].innerHTML;
				}
			}
		}
	}
	...
	this.innerHTML = "Added to the Cart!";
}
